      ...
      {view === 'HOME' && (
        <div className="animate-in fade-in flex flex-col min-h-screen">
          ...
          <main className="max-w-6xl mx-auto w-full px-6 flex-1 -mt-10 relative z-30 pb-40">
             ...
             {/* 2. Nossos Rituais - APENAS CATEGORIAS */}
             <section className="mb-24" id="catalogo">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                   <h2 className={`text-2xl font-black font-display italic flex items-center gap-6 ${theme === 'light' ? 'text-zinc-900' : 'text-white'}`}>Nossos Rituais <div className="h-1 w-10 gradiente-ouro opacity-10"></div></h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                   {categories.filter(cat => cat !== 'Todos').map(cat => {
                     ...
                     return (
                       <div key={cat} className={`rounded-[2.5rem] p-8 space-y-6 shadow-xl hover:scale-[1.02] transition-all group cursor-pointer ${theme === 'light' ? 'bg-white border border-zinc-200 hover:border-blue-300' : 'cartao-vidro border-white/5 hover:border-[#D4AF37]/30'}`}>
                          <div className="flex items-start justify-between">
                             <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all ${theme === 'light' ? 'bg-blue-50 group-hover:bg-blue-100' : 'bg-[#D4AF37]/10 group-hover:bg-[#D4AF37]/20'}`}>
                                <Scissors className={theme === 'light' ? 'text-blue-600' : 'text-[#D4AF37]'} size={32}/>
                             </div>
                             <span className={`text-2xl font-black font-display ${theme === 'light' ? 'text-blue-600' : 'text-[#D4AF37]'}`}>
                               {minPrice === maxPrice ? `R$ ${minPrice}` : `R$ ${minPrice} - ${maxPrice}`}
                             </span>
                          </div>
                          <div className="space-y-2">
                             <h3 className={`text-xl font-black font-display italic ${theme === 'light' ? 'text-zinc-900' : 'text-white'}`}>{cat}</h3>
                             <p className={`text-[10px] font-black uppercase tracking-widest ${theme === 'light' ? 'text-zinc-600' : 'text-zinc-500'}`}>{categoryServices.length} {categoryServices.length === 1 ? 'serviço' : 'serviços'}</p>
                          </div>
                          <div className="pt-4 flex items-center justify-between">
                             <div className={`flex items-center gap-2 text-[10px] font-black uppercase ${theme === 'light' ? 'text-zinc-600' : 'text-zinc-500'}`}>
                                <Sparkles size={14}/> Premium
                             </div>
                             <button onClick={() => {setView('BOOKING'); setPasso(2);}} className="gradiente-ouro text-black px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg hover:scale-105 transition-all">Agendar</button>
                          </div>
                       </div>
                     );
                   })}
                   ...
                   {professionals.map(prof => (
                      <div key={prof.id} className={`rounded-[2rem] p-6 text-center space-y-4 group transition-all hover:scale-105 ${theme === 'light' ? 'bg-white border border-zinc-200 hover:border-blue-300' : 'cartao-vidro border-white/5 hover:border-[#D4AF37]/30'}`}>
                         <div className="relative mx-auto w-24 h-24">
                            <img src={prof.avatar} className="w-full h-full rounded-2xl object-cover border-2 border-[#D4AF37]" alt="" />
