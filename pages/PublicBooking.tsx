...
const PublicBooking: React.FC<PublicBookingProps> = ({ initialView = 'HOME' }) => {
  ...
  const renderClientDashboard = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-12 pb-32 animate-in fade-in duration-700">
        ...
        <div className="space-y-6">
           <h3 className="text-xl font-black italic flex items-center gap-3 text-white"><MessageSquare className="text-[#D4AF37]"/> Minhas Sugestões & Respostas</h3>
           <div className="grid grid-cols-1 gap-4">
              {mySuggestions.map(sug => (
                 <div key={sug.id} className="cartao-vidro p-6 rounded-[2rem] border-white/5 space-y-4">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[9px] font-bold text-zinc-600 uppercase">{sug.date}</span>
                    </div>
                    {sug.reply && (
                      <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/20 p-4 rounded-2xl space-y-2 animate-in fade-in zoom-in duration-500">
                        <p className="text-[10px] font-black uppercase text-[#D4AF37] flex items-center gap-2">
              ...
              <div className="mt-8 space-y-4">
                <textarea 
                  ...
                  rows={3}
                />
                <button onClick={handleAddSuggestion} className="gradiente-ouro text-black px-8 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2">
    ...
    <div className="relative pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto space-y-32">
      <div className="text-center space-y-8 relative">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 pt-10">
          <button onClick={() => setView('BOOKING')} className="px-12 py-6 gradiente-ouro rounded-2xl font-black uppercase tracking-widest text-sm flex items-center gap-3">Agendar Agora <ArrowRight size={20}/></button>
          <button onClick={() => setView(user ? 'CLIENT_DASHBOARD' : 'LOGIN')} className="px-12 py-6 bg-white/5 border border-white/10 rounded-2xl text-white font-black uppercase tracking-widest text-sm">
  ...
  return (
    <div className={`min-h-screen ${theme === 'light' ? 'bg-[#F8F9FA]' : 'bg-[#050505]'}`}>
      {view !== 'HOME' && (
        <button onClick={() => setView('HOME')} className="fixed top-8 left-8 z-[150] p-4 bg-white/5 border border-white/10 rounded-2xl text-white">
          <ChevronLeft size={24}/>
        </button>
      )}
      ...
      {/* Visualizações de agendamento e login devem seguir sua lógica original */}
      {view === 'BOOKING' && <div className="pt-32 px-4 text-center text-white"><h2>Página de Agendamento</h2></div>}
      {view === 'LOGIN' && <div className="pt-32 px-4 text-center text-white"><h2>Login do Membro</h2></div>}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
           <div className="cartao-vidro w-full max-w-lg rounded-[3.5rem] p-10 border-white/10 space-y-10">
              ...
              <div className="flex gap-4">
                 ...
                 <button onClick={handleAddReview} className="flex-1 gradiente-ouro text-black py-5 rounded-xl text-[10px] font-black uppercase">Enviar</button>
              </div>
           </div>
        </div>
      )}
