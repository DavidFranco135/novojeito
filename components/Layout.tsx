...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${theme === 'light' ? 'bg-[#F3F4F6]' : 'bg-[#050505]'}`}>
      ...
      {/* Sidebar - Agora com inversão de cores para modo claro */}
      <aside className={`fixed inset-y-0 left-0 z-50 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 transition-all duration-300 ease-in-out ${isCollapsed ? 'lg:w-20' : 'lg:w-72'} ${theme === 'light' ? 'bg-white border-r border-zinc-200' : 'bg-[#0A0A0A] border-r border-white/5'}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 flex items-center justify-between">
            ...
            <button onClick={() => setIsCollapsed(!isCollapsed)} className={`hidden lg:flex p-2 rounded-xl transition-all ${theme === 'light' ? 'text-zinc-400 hover:bg-zinc-100' : 'text-zinc-600 hover:bg-white/5'}`}>
          ...
          <div className={`p-4 border-t ${theme === 'light' ? 'border-zinc-200' : 'border-white/5'}`}>
            <button onClick={logout} className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl transition-all ${theme === 'light' ? 'text-red-600 hover:bg-red-50' : 'text-zinc-500 hover:bg-red-500/10 hover:text-red-500'}`}>
              <LogOut size={20} />
              {!isCollapsed && <span className="text-xs font-black uppercase tracking-widest">Sair do Painel</span>}
            </button>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header - Corrigido para inverter cores no modo claro */}
        <header className={`h-24 flex items-center justify-between px-4 md:px-10 border-b ${theme === 'light' ? 'bg-white border-zinc-200' : 'bg-[#050505]/80 backdrop-blur-md border-white/5'}`}>
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(true)} className={`lg:hidden p-3 rounded-xl ${theme === 'light' ? 'bg-zinc-100 text-zinc-900' : 'bg-white/5 text-white'}`}>
              <Menu size={20} />
            </button>
            <div className="hidden md:block">
               <h2 className={`text-sm font-black uppercase tracking-[0.2em] ${theme === 'light' ? 'text-zinc-400' : 'text-zinc-500'}`}>Bem-vindo de volta,</h2>
               <p className={`text-xl font-black italic ${theme === 'light' ? 'text-zinc-900' : 'text-white'}`}>{user?.name || 'Administrador'}</p>
            </div>
          </div>

          <div className="flex items-center gap-3 md:gap-6">
            {/* Botão de Troca de Tema - Estilizado como na página pública */}
            <button 
              onClick={toggleTheme}
              className={`p-3 md:p-4 rounded-2xl transition-all shadow-sm ${
                theme === 'light' 
                  ? 'bg-zinc-100 text-[#D4AF37] hover:bg-zinc-200' 
            ...
            <div className="relative">
              <button onClick={() => setShowNotifs(!showNotifs)} className={`p-3 md:p-4 rounded-2xl relative transition-all ${theme === 'light' ? 'bg-zinc-100 text-zinc-900 hover:bg-zinc-200' : 'bg-white/5 text-white hover:bg-white/10'}`}>
                <Bell size={20} />
                {notifications.filter(n => !n.read).length > 0 && (
                  <span className="absolute top-3 right-3 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#050505]"></span>
                )}
              </button>
              ...
              {showNotifs && (
                <>
                  ...
                  <div className={`absolute right-0 mt-4 w-80 md:w-96 rounded-[2rem] shadow-2xl z-50 overflow-hidden border animate-in slide-in-from-top-2 ${theme === 'light' ? 'bg-white border-zinc-200' : 'bg-[#0A0A0A] border-white/10'}`}>
                    <div className="p-6 border-b border-white/5 flex items-center justify-between bg-gradiente-ouro-dark">
                    ...
                    <div className="max-h-[400px] overflow-y-auto scrollbar-hide">
                       {notifications.length === 0 && <p className="p-10 text-center text-[10px] font-black uppercase text-zinc-500 italic">Nada por aqui.</p>}
                       {notifications.map(n => (
