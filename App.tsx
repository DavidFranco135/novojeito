          ...
          {!isRegistering ? (
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <input type="email" placeholder="E-mail (opcional)" value={registerData.email} onChange={e => setRegisterData({...registerData, email: e.target.value})} className={`w-full border p-5 rounded-2xl outline-none focus:border-[#D4AF37] font-bold ${theme === 'light' ? 'bg-white border-zinc-200' : 'bg-white/5 border-white/10'}`} />
    ...
    return (
      <div className={`relative min-h-screen theme-transition ${theme === 'light' ? 'bg-[#F8F9FA]' : 'bg-[#050505]'}`}>
        <div className="fixed bottom-8 left-8 z-[100] flex gap-3">
          ...
          <button onClick={() => setIsPublicView(false)} className={`bg-zinc-900 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] border border-white/10 shadow-2xl hover:bg-zinc-800 transition-all flex items-center gap-4`}>
             <LogOut size={16}/> Sair da Visão Cliente
          </button>
        </div>
        <PublicBooking />
      </div>
